...
: doxt77   \ move spp to sp once we have it
          ...
          xfd @ fxb ! xgd @ gxb ! xhd @ hxb ! xid @ ixb ! xjd @ jxb !
                                                   ;
: dos4     ( n1 - n2 )  \ xor b with c move to s2a
     axb @ axc @ xor  xad !
     bxb @ bxc @ xor  xbd !
     cxb @ cxc @ xor  xcd !
     dxb @ dxc @ xor  xdd !
...
: dotx88 \ create new S2  xor a with move to c
          axa @ axb @ xor axc !
          bxa @ bxb @ xor bxc !
          cxa @ cxb @ xor cxc !
          dxa @ dxb @ xor dxc !
          ...
          jxa @ jxb @ xor jxc !
                              ;
: 3dot \ create new S2 internediate
          axa @ axb @ xor axc !
          bxa @ bxb @ xor bxc !
          cxa @ cxb @ xor cxc !
          dxa @ dxb @ xor dxc !
                                                     ...
                                                     xhd @  0=  if ." it's a zero @ 8" cr
                                                        ...
                                                        xid @  0=  if  ." it's a zero @ 9" cr
                                                             ...
                                                             xjd @  0=  if  ." it's a zero @ 10" cr
                                                                                       ...
                                                                                       then  ;
: cc10  \ looks for our number on temp S2 to exeute on AXB the word is then move via doxt77 to axb then do  cx3 for an xt11a
        \ which updates S2 to prveno for loop search



                        cr    axc @  0=  if  ." it's a zero @ 1" cr
                                             else   ." its 1"  cr \ 1   xt1   cr
                                                 then
                                 bxc @   0=  if  ." it's a zero @ 2 " cr
                                               else  ." its 2" cr  \  2 xt1  cr
                                                    then
                                    cxc @   0=  if   ." it's a zero @ 3" cr
                                                 else  ." its 3"  cr \  3 xt1 cr
                                                        then
                                        dxc @  0=  if   ." it's a zero @ 4" cr
                                                   else  ." its 4"  cr \  4 xt1 cr
                                                            then
                                           exc @  0=  if   ." it's a zero @ 5" cr
                                                      else  ." its 5"  cr \ 5 xt1 cr
                                                                 then
                                              fxc @  0=  if  ." it's a zero @ 6" cr
                                                         else  ." its 6"  cr \ 6 xt1 cr
                                                                    then
                                                 gxc @  0=  if  ." it's a zero @ 7" cr
                                                           else  ." its 7"  cr \  7 xt1 cr
                                                                        then
                                                     hxc @  0=  if ." it's a zero @ 8" cr
                                                              else  ." its a 8" cr \ 8 xt1 cr
                                                                           then
                                                        ixc @  0=  if  ." it's a zero @ 9" cr
                                                                 else  ." its 9"  cr \  9 xt1 cr
                                                                              then
                                                             jxc @  0=  if  ." it's a zero @ 10" cr
                                                                    else  ." its 0"  cr \ 10 xt1 cr
                                                                                       then  ;
: cxww \ looks for our number on temp S2 to exeute on AXB the word is then move via doxt77 to axb then do  cx3 for an xt11a
        \ which updates S2 to prveno for loop search



                          cr    xad @  0=  if  ." it's a zero @ 1" cr
                                             else   ." its 1"  cr \ 1   xt11a   cr
                                                 then
                                 xbd @   0=  if  ." it's a zero @ 2 " cr
                                               else  ." its 2" cr \ 2 xt11a  cr
                                                    then
                                    xcd @   0=  if   ." it's a zero @ 3" cr
                                                 else  ." its 3" cr \   3 xt11a cr
                                                        then
                                        xdd @  0=  if   ." it's a zero @ 4" cr
                          ...
                          cr   axb @ xad @  xor 0=  if  ." it's a zero @ 1" cr
                                 ...
                                 bxb @ xbd @  xor 0=  if  ." it's a zero @ 2 " cr
                                    ...
                                    cxb @ xcd @  xor 0=  if   ." it's a zero @ 3" cr
                                                        ...
                                                        then
                                        dxb @ xdd @  xor 0=  if   ." it's a zero @ 4" cr
                                                   else  ." its 4" cr \   4 xt11a cr
                                                            then
                                           exb @ xed @  xor 0=  if   ." it's a zero @ 5" cr
                                                      else  ." its 5" cr \  5 xt11a cr
                                                                 then
                                              fxb @ xfd @  xor 0=  if  ." it's a zero @ 6" cr
                                                         else  ." its 6"  cr \  6 xt11a cr
                                                                    then
                                                 gxb @ xgd @  xor 0=  if  ." it's a zero @ 7" cr
                                                           else  ." its 7"  cr \  7 xt11a cr
                                                                        then
                                                     hxb @ xhd @  xor 0=  if ." it's a zero @ 8" cr
                                                              else  ." its a 8" cr \ 8 xt11a cr
                                                                           then
                                                        ixb @ xid @  xor 0=  if  ." it's a zero @ 9" cr
                                                                 else  ." its 9"   cr \ 9 xt11a cr
                                                                              then
                                                             jxb @ xjd @  xor 0=  if  ." it's a zero @ 10" cr
                                                                    else  ." its 0"  cr \ 10 xt11a cr
                                                                                       then  ;
